...
const BASE_URL = process.env.BASE_URL_SDK
  ...
  BASE_URL_SECURE = process.env.SECURE_URL_SDK
    ? process.env.SECURE_URL_SDK
    : "https://secure.payco.co",
  ENTORNO = process.env.ENTORNO_SDK ? process.env.ENTORNO_SDK : "/restpagos",
  BASE_URL_APIFY = process.env.BASE_URL_APIFY
  ...
  if (apify) {
    ...
    for (var key in data) {
      if (data.hasOwnProperty(key) && key.includes("extras_epayco")) {
        set[langkey(key)] = {
          extra5: encrypt(data[key]["extra5"], privateKey),
        };
      } else {
